...
def alvo_do_elo(m: mujoco.MjModel, d: mujoco.MjData, c: Cena, elo: int) -> np.ndarray:
    ...
    `comando._alvo_ancorado_na_base` sobrescreve `a[:, 2] = altura_carregar` (0,90 m)
    ...
    ⚠ NO TREINO O z É SORTEADO por episódio em `altura_carregar_faixa` (0,85 a 0,95).
    Aqui ele é DETERMINÍSTICO e vale o centro da faixa, que é o `altura_carregar` do
    `.npz`. Este arquivo não tem env, portanto não tem sorteio a reproduzir.
