...
# ⚠ `giro_indevido` SAIU em 21/08, no mesmo dia em que entrou. Registro do erro:
#
# O fabricante exclui o z do `body_angular_velocity_penalty` DE PROPÓSITO — o
# comentário no fonte é literal: `# Don't penalize z-angular velocity.` Eu li isso
# como um vão e escrevi um termo para fechá-lo, gateado em "|ωz_cmd| < 0,05".
#
# Três medidas mostraram que o termo era desnecessário e mal desenhado:
#
# 1. MAGNITUDE. A ωz = 0,556 ele custava 0,5 × 0,309 = 0,155/s em ~40% das amostras,
#    ou ~0,06/s. O `action_rate_l2` no mesmo instante custava 1,49/s — vinte e cinco
#    vezes mais. O termo não movia nada.
#
# 2. ESCOPO. Com `heading_command = True`, um env que já alinhou o rumo anda RETO com
#    ωz_cmd ≈ 0. Portanto o gate "ninguém pediu giro" pegava ~40% das amostras de
#    marcha, e punia a contra-rotação de tronco que toda passada precisa. Era
#    exatamente o que o comentário do fabricante avisava.
#
# 3. QUEM CONSERTOU O YAW. A `razao_giro` foi de 0,0199 (bloco 2) para 0,373 (bloco
#    3). A outra mudança no eixo do yaw foi o reset ±3,14, que é a receita do
#    fabricante e não invenção. O crédito é dela.
#
# E um defeito de projeto que também sai com ele: a EMA `poc_erro_giro_ema` vivia
# DENTRO deste termo, e o `reward_manager.py:122` pula termos com peso zero.
# Portanto desligar a penalidade matava a medição em silêncio. Medição não mora
# dentro de penalidade.
